...
void
SeqTsHeader::SetPsn (uint32_t psn)
{
  HPS.psn = psn;
}
uint32_t
SeqTsHeader::GetPsn (void) const
{
  return HPS.psn;
}
...
uint32_t SeqTsHeader::GetHeaderSize(void){
	return sizeof(m_seq)+sizeof(m_pg) + sizeof(HPS.psn)+ IntHeader::GetStaticSize();
}
