...
int SwitchNode::getRandomInteger(int min, int max) {
	// 使用系统时钟作为随机数种子
...
#ifdef SWITCH_SPRAY
	uint32_t idx = getRandomInteger(0,nexthops.size()-1);
#else
...
void SwitchNode::CheckAndSendPfc(uint32_t inDev, uint32_t qIndex){
	...
	if (m_mmu->CheckShouldPause(inDev, qIndex)){
		...
		if(Simulator::Now().GetInteger() > last_pfc_print + 1000000){
			std::cout<<Simulator::Now().GetSeconds() << " PFC_TANGJIAN_SEND IssueNode:"<<GetId()<<" ToDevice:"<<inDev<<std::endl;
			last_pfc_print = Simulator::Now().GetInteger();
		}
...
std::unordered_map<uint32_t, std::vector<int> > * SwitchNode::GetTableEntry(){
	return &m_rtTable;
}

Ptr<NetDevice> SwitchNode::GetNetDeviceByID(uint32_t id){
	return m_devices[id];
}
