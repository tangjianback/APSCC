...
#define QUEUE_INFO
#define BANDWIDTH_TIME_INTVA 1000000
// #define RECEVE_BW_INFO
namespace ns3 {
...
public:
	...
	void PCNGenerateCNP(Ptr<RdmaRxQueuePair> rxQp);
	...
	/*CTCC*/
	uint32_t m_updateRateBytesIntervalCtcc;
	double m_rateDecreaseAlphaCtcc;  // for RTT
	DataRate m_rateDeltaCtcc;
	double m_rminRatioCtcc;		//< Rate of hyper-additive increase/bw
	double m_rinitRatioCtcc;		//init rate ratio
	double m_gammaCtcc;
	double m_targetWeightCtcc;
	double m_targetBiasCtcc;
	void CnpReceivedCtcc(Ptr<RdmaQueuePair> qp);
	void NackReceivedCtcc(Ptr<RdmaQueuePair> qp);
	void HandleAckCTCC(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
	void UpdateRateCtccC(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch);
	void RateIncCTCC(Ptr<RdmaQueuePair> qp);
	...
	void HandleAckSwift(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch,Ptr<QbbNetDevice> dev);
	void HandleAckSwift_finegrained(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch,Ptr<QbbNetDevice> dev);
	void HandleAckPCN(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch,Ptr<QbbNetDevice> dev);
	void HandleAckHPS_CC_ACK(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch,Ptr<QbbNetDevice> dev);
	void HandleAckHPS_CC_TIMEER(Ptr<RdmaQueuePair> qp,Ptr<QbbNetDevice> dev);
	uint64_t Get_targetdelay_forfabric_swift(Ptr<RdmaQueuePair> qp);
	double Get_ecn_threshold_hps(Ptr<RdmaQueuePair> qp);
	uint64_t Mix(double a, double b);
	uint64_t Max(double a, double b);
	void UpdateWindow_forfabric(Ptr<RdmaQueuePair> qp);
	void UpdateWindow_forfabric_finegrained(Ptr<RdmaQueuePair> qp);
	void ChangeWindow(Ptr<RdmaQueuePair> qp);
	 ...
	 * CTCCLOSSY
	 *********************/
	#define CCPFL_EPSILON (0)
	#define ctcclossy_unit(a) ((a) >> CCPFL_EPSILON)
	void ctcclossy_recv_ack_handle(Ptr<RdmaQueuePair> qp, Ptr<Packet> p, CustomHeader &ch, Ptr<QbbNetDevice> dev);
	void ctcclossy_update_realtime_rate(ctcclossy_algo_s &algo_ctx);
	void ctcclossy_all_ev_deal(Ptr<RdmaQueuePair> qp, cc_ev_type_s ev_type, cc_event_data_s &ev_data);
	void ctcclossy_ev_init(ctcclossy_algo_s &algo_ctx, cc_event_data_s &ev_data);
	void ctcclossy_ev_resp(Ptr<RdmaQueuePair> qp, cc_event_data_s &ev_data);
