...
namespace ns3 {
	...
	/*
		网卡发送数据包的主要实现逻辑，每个QP都根据当前的发送速率维护一个发送时间间隔，即计算QP下一个包的发送时间戳。
		DequeueAndTransmit函数会寻找所有QP中下一个包发送的时间戳和目前的线程时间是否一致，如果一致代表可以发送数据。并更新QP的下一个数据包的发送时间戳。
		如果没有当前时间可以发送的数据包，则计算最近可发送QP的时间，并设置DequeueAndTransmit事件到对应事件。
	*/
	void
		QbbNetDevice::DequeueAndTransmit(void)
	{
		...
		Ptr<Packet> p;
		if (m_node->GetNodeType() == 0){   //RNIC
			int qIndex = m_rdmaEQ->GetNextQindex(m_paused); //find next avalble qp
			if (qIndex != -1024){
				if (qIndex == -1){ // high prio
