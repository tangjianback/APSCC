...
void CustomHeader::Serialize (Buffer::Iterator start) const{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  i.WriteU16(ack.pg);
		  i.WriteU32(ack.seq);
		  i.WriteU16(ack.HPS.ooo_distance);
		  i.WriteU32(ack.HPS.ack_psn);
		  i.WriteU64(ack.PCN.receiveing_rate);
...
CustomHeader::Deserialize (Buffer::Iterator start)
{
  ...
  if (headerType & L4_Header){
	  if (l3Prot == 0x6){ // TCP
		  ...
		  ack.pg = i.ReadU16();
		  ack.seq = i.ReadU32();
		  ack.HPS.ooo_distance = i.ReadU16();
		  ack.HPS.ack_psn = i.ReadU32();
		  ack.PCN.receiveing_rate = i.ReadU64();
		  if (getInt)
...
uint32_t CustomHeader::GetAckSerializedSize(void){
	return sizeof(ack.sport) + sizeof(ack.dport) + sizeof(ack.flags) + sizeof(ack.pg) + sizeof(ack.seq) +sizeof(ack.HPS.ooo_distance)+sizeof(ack.HPS.ack_psn) +sizeof(ack.PCN.receiveing_rate)+ IntHeader::GetStaticSize();
}

uint32_t CustomHeader::GetUdpHeaderSize(void){
	return 8 + sizeof(udp.pg) + sizeof(udp.seq) + sizeof(udp.HPS.psn) + IntHeader::GetStaticSize();
}
